# Desktop Wireframes - Weekly Meal Planner for Families

## Navigation Structure
```
+-----------------------------------------------------------------------+
|  LOGO                                      MENU LINKS       BUTTON CTA |
+-----------------------------------------------------------------------+
```

## Hero Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|  +-----------------------------------+  +-------------------------+   |
|  |                                   |  |                         |   |
|  |  HEADLINE                         |  |                         |   |
|  |  Subheadline                      |  |                         |   |
|  |                                   |  |                         |   |
|  |  Description text                 |  |      HERO IMAGE         |   |
|  |                                   |  |                         |   |
|  |  [Feature Icons w/ Labels]        |  |                         |   |
|  |                                   |  |                         |   |
|  |  [CTA BUTTON]                     |  |                         |   |
|  |  No commitment text               |  |                         |   |
|  +-----------------------------------+  +-------------------------+   |
|                                                                       |
+-----------------------------------------------------------------------+
```

## Features Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|                       FEATURES SECTION HEADLINE                       |
|                       Subheadline description                         |
|                                                                       |
|  +-------------+    +-------------+    +-------------+                |
|  |    ICON     |    |    ICON     |    |    ICON     |                |
|  |             |    |             |    |             |                |
|  |   Feature   |    |   Feature   |    |   Feature   |                |
|  |  Title #1   |    |  Title #2   |    |  Title #3   |                |
|  |             |    |             |    |             |                |
|  | Description | ↔  | Description | ↔  | Description |                |
|  |    text     |    |    text     |    |    text     |                |
|  +-------------+    +-------------+    +-------------+                |
|                                                                       |
|  +-------------+    +-------------+    +-------------+                |
|  |    ICON     |    |    ICON     |    |    ICON     |                |
|  |             |    |             |    |             |                |
|  |   Feature   |    |   Feature   |    |   Feature   |                |
|  |  Title #4   |    |  Title #5   |    |  Title #6   |                |
|  |             |    |             |    |             |                |
|  | Description | ↔  | Description | ↔  | Description |                |
|  |    text     |    |    text     |    |    text     |                |
|  +-------------+    +-------------+    +-------------+                |
|                                                                       |
+-----------------------------------------------------------------------+
```

## How It Works Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|                     HOW IT WORKS SECTION HEADLINE                     |
|                       Subheadline description                         |
|                                                                       |
|  +-------------+    +-------------+    +-------------+                |
|  |     [1]     |    |     [2]     |    |     [3]     |                |
|  |    ICON     |    |    ICON     |    |    ICON     |                |
|  |             |    |             |    |             |                |
|  |   Step 1    |    |   Step 2    |    |   Step 3    |                |
|  |   Title     | -> |   Title     | -> |   Title     |                |
|  |             |    |             |    |             |                |
|  | Description |    | Description |    | Description |                |
|  |    text     |    |    text     |    |    text     |                |
|  +-------------+    +-------------+    +-------------+                |
|                                                                       |
+-----------------------------------------------------------------------+
```

## Testimonials Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|                     TESTIMONIALS SECTION HEADLINE                     |
|                                                                       |
|  +-----------------------------------+  +-------------------------+   |
|  |                                   |  |                         |   |
|  |  "Testimonial quote text..."      |  |                         |   |
|  |                                   |  |                         |   |
|  |  [Photo]  Name                    |  |      TESTIMONIAL        |   |
|  |           Role/Description        |  |         IMAGE           |   |
|  |                                   |  |                         |   |
|  +-----------------------------------+  +-------------------------+   |
|                                                                       |
|  [Testimonial Navigation Dots]                                        |
|                                                                       |
+-----------------------------------------------------------------------+
```

## Pricing Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|                      PRICING SECTION HEADLINE                         |
|                       Subheadline description                         |
|                                                                       |
|  +-----------------------------+  +-----------------------------+     |
|  |                             |  |                             |     |
|  |        FREE TRIAL           |  |        MONTHLY PLAN         |     |
|  |                             |  |                             |     |
|  |         14 DAYS             |  |         R$9.90/mo           |     |
|  |                             |  |                             |     |
|  | • Feature point             |  | • Feature point             |     |
|  | • Feature point             |  | • Feature point             |     |
|  | • Feature point             |  | • Feature point             |     |
|  | • Feature point             |  | • Feature point             |     |
|  |                             |  |                             |     |
|  |      [START TRIAL]          |  |      [SUBSCRIBE NOW]        |     |
|  |                             |  |                             |     |
|  +-----------------------------+  +-----------------------------+     |
|                                                                       |
+-----------------------------------------------------------------------+
```

## CTA Section
```
+-----------------------------------------------------------------------+
|                                                                       |
|                         CTA SECTION HEADLINE                          |
|                          Compelling subtext                           |
|                                                                       |
|                           [SIGN UP FORM]                              |
|                                                                       |
|  +---------------------------------------------------------------+   |
|  |  Name:        [                                            ]  |   |
|  |                                                               |   |
|  |  WhatsApp:    [                                            ]  |   |
|  |                                                               |   |
|  |  Preferences: [                    ▼                       ]  |   |
|  |                                                               |   |
|  |                      [START FREE TRIAL]                       |   |
|  +---------------------------------------------------------------+   |
|                                                                       |
|                    No credit card required • Cancel anytime           |
|                                                                       |
+-----------------------------------------------------------------------+
```

## Footer
```
+-----------------------------------------------------------------------+
|                                                                       |
|  LOGO                                                                 |
|  Tagline                                                              |
|                                                                       |
|  ABOUT US         LEGAL            CONTACT                            |
|  • Link           • Privacy        • Email                            |
|  • Link           • Terms          • WhatsApp                         |
|  • Link           • Cookies        • Social Media                     |
|                                                                       |
|  © 2025 Weekly Meal Planner. All rights reserved.                     |
|                                                                       |
+-----------------------------------------------------------------------+
```
